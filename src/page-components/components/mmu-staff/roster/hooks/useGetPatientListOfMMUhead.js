...
const useGetPatientListOfMMUhead = ({ currentPage, selectedHeadID }) => {
  ...
  const getPatientList = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(
        ...
        {
          params: { mmuHead: selectedHeadID, page: currentPage - 1 },
          headers: {
  ...
  }, [currentPage, selectedHeadID, navigate]);

  useEffect(() => {
    if (selectedHeadID !== null) {
      getPatientList();
    }
  }, [currentPage, getPatientList, selectedHeadID]);
...
export default useGetPatientListOfMMUhead;
